...
export function LocalSettings({ onRefresh }: { onRefresh: () => void }) {
  ...
  const selectProfile = (value?: DesktopModelProfile) => {
    ...
    setDeletePending(false); setEditorOpen(true);
  };
  useEffect(() => {
    void window.vistoraDesktop?.localStatus().then((value) => {
      setStatus(value);
      if (value) { selectProfile(value.profiles.find(p => p.id === value.activeProfileId)); setEditorOpen(!value.enabled || !!value.issue); }
  ...
  if (!status) return <section className="settings-section local-runtime-settings"><p role={message ? "alert" : "status"}>{message || (window.vistoraDesktop ? "正在读取本地模型设置…" : "请在桌面应用中管理本地模型设置。")}</p></section>;
  ...
  const configure = async (enabled: boolean) => {
    ...
    try {
      ...
      if (!result.ok) setMessage(result.error || "AI 设置未保存");
        ...
        if (enabled) { setProfileId(result.status!.activeProfileId); setEditorOpen(false); }
  ...
  return <>
    <section className="settings-section local-runtime-settings">
      ...
      <details className="settings-explainer"><summary>支持的接口与数据使用说明</summary><p>统一使用 OpenAI 兼容的 Chat Completions API。供应商的处理地区、保留期限和训练使用规则由其决定，本应用不保证零保留或禁止训练。原生 Anthropic / Gemini 等其他协议需使用兼容接口。</p></details>
      {status.issue && <p role="alert">{status.issue}</p>}
      {status.profiles.length > 0 && <div className="model-library">
      ...
      <details className="model-editor-disclosure" open={editorOpen} onToggle={event => setEditorOpen(event.currentTarget.open)}><summary>{selected ? "修改模型配置" : "连接一个模型"}<span>地址、密钥与模型名称</span></summary>
      ...
      <p className="api-protocol-note">OpenAI 兼容接口 · Chat Completions</p>
      ...
      <div className="settings-actions"><button className="primary-action" disabled={busy || !consent || !name.trim() || !baseUrl.trim() || !model.trim() || (!key.trim() && !canReuseKey)} onClick={() => void configure(true)}>{busy ? "正在处理…" : "保存并切换到此模型"}</button></div>
      <small>密钥由 Windows 当前账户加密保护，不随备份导出。关闭 AI 不会删除已有认识。</small>
      </details>
      {status.enabled && <div className="settings-actions model-power-action"><button disabled={busy} onClick={() => void configure(false)}>关闭 AI 并移除当前密钥</button></div>}
    </section>
    <details className="settings-section local-runtime-settings backup-settings"><summary className="settings-section-heading"><div><span><strong>备份与恢复</strong><small>导出或恢复加密备份</small></span></div></summary>
